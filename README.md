...
EC2 is a service that provides virtual servers called instances.These instances can be used to run applications, websites, or backend services.EC2 allows us to choose the operating system,instance size and network configuration.
...
Subnet
A subnet is a smaller network inside a VPC.Subnets can be public or private.Public subnets allow internet access while private subnets are used for internal resources that should not be exposed directly.
...
This process helped in understanding each configuration option available while creating an EC2 instance.

EC2 Creation Using Terraform
 Steps Followed:
1. Installed Terraform in my PC
2. Configured AWS CLI using aws configure command.
